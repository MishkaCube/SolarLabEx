...
public class AddBirthday
{
    ...
    //Add a new entry
    public void Add() throws ClassNotFoundException, NoSuchMethodException,
            InvocationTargetException, InstantiationException, IllegalAccessException, SQLException
    {
        ...
        try
        {
            ...
            Names.add(name);
            con.AddToDB(name, LocalDate.of(year, month, day).toString());

            System.out.println("Новый день рождения добавлен в базу данных! :)");
    ...
    public void Remove() throws ClassNotFoundException, NoSuchMethodException,
            InvocationTargetException, InstantiationException, IllegalAccessException, SQLException
    {
        if (birthdays.size() == 0) {
    ...
    public void EditList() throws ClassNotFoundException, NoSuchMethodException,
            InvocationTargetException, InstantiationException, IllegalAccessException, SQLException
    {
        if (birthdays.size() == 0) {
            System.out.println("Изменять некого, список пуст.");
        } else {
                    ...
                    if (employee.equals(Names.get(i)))
                    {
                        ...
                        if (choose <= 2 && choose > 0) {
                            if (choose == 1)
                            {
                                ...
                                //Names.remove(i);
                                con.EditDB(new_name, "first_name", Names.get(i));
                            ...
                            if (choose == 2) {
                                ...
                                con.EditDB(LocalDate.of(year, month, day).toString(), "birthday_date", LocalDate.of(year, month, day).toString());
